...
class ComfoAir:
    ...
    def __init__(self, host=None, port=0, serialport=None, kwltype='comfoair350'):
        ...
        if kwltype in commands.controlset and kwltype in commands.commandset:
            ...
            # Add all commands to _params 
            for commandname, commandcode in self._commandset.items():
                #self.log_debug('Commandname: {} Commandcode {}'.format(commandname, commandcode['Command']))
                if not commandcode['Command'] in self._params:
                    self._params[commandcode['Command']] = {'commandname': [commandname]}
                else:
                    self._params[commandcode['Command']]['commandname'].append(commandname)
                
        else:
            self.log_err('Commands for KWL type \'{}\' could not be found!'.format(kwltype))
            return None
        #self.log_debug('self._params \'{}\''.format(self._params))
    ...
    def handle_cyclic_cmds(self):
        ...
        self.log_debug('Handle cyclic commands: {}'.format(self._cyclic_cmds.keys()))
    ...
    def send_command(self, commandname, value=None):
        ...
        try:
            ...
            if 'ValueTransform' in commandconf and value is not None and value != '' and commandtype == 'Write':
                ...
                self.log_debug('Transforming value using method {} from {}'.format(commandtransform, value))
            ...
            if commandtype == 'Read':
                ...
                # Parse response
                self.log_debug('Parsing response')
                responses = self.parse_response(packet)
    ...
    def parse_response(self, response):
        ...
        if (receivedchecksum != checksum):
            ...
            self.log_debug('Calculated checksum matches received checksum.')

        #self.log_debug('Self params: {}'.format( self._params))
        #self.log_debug('Commandcode: {}'.format( commandcode))        
        # Find Commandcode in in Command Dictionairy
        ...
        if commandcode in self._params.keys():
            #self.log_debug('Iterating self._params ccode {}.'.format(ccode))
           
            #self.log_debug('Iterating Commandcode {}.'.format(commandcode))
            # Iterate over all corresponding items
            for i in range(0, len(self._params[commandcode]['commandname'])):
                ...
                if 'ValueTransform' in commandconf:
                    commandtransform = commandconf['ValueTransform']
                valuebit = 0
                if 'ValueBit' in commandconf:
                    valuebit = int(commandconf['ValueBit'])
                ...
                if index + commandvaluebytes <= len(databytes):
                    ...
                    # Tranform value
                    value = self.value_transform(rawvalue, commandtype, commandtransform, valuebit)
                    ...
                    responses[commandname.replace("Read","")]=value
                    # Update item
                    #item(value, 'ComfoAir')
                else:
                    self.log_err('Telegram did not contain enough data bytes for the configured command {} to extract a value!'.format(commandname))
        else:
           self.log_err('Cannot find commandcode {:02X} in _params.keys() {}!'.format(commandcode, self._params.keys())) 
        return responses
    ...
    def value_transform(self, value, commandtype, transformmethod,bitpos=0):
        self.log_debug('Transforming value {} commandtype {} transformmethod {}'.format(value, commandtype, transformmethod))
        if transformmethod == 'Temperature':
            if commandtype == 'Read':
                return value / 2 - 20
            elif commandtype == 'Write':
                return (int(value) + 20) * 2
        elif transformmethod == 'RPM':
            if commandtype == 'Read':
                return int(1875000 / value)
            elif commandtype == 'Write':
                return int(1875000 / value)
        elif transformmethod == 'Bitarray':
            return str(bin(value)[2:].zfill(8))
        elif transformmethod == 'Bit':
            return int(bin(value)[2:].zfill(8)[bitpos])
        return value
